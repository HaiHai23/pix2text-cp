...
class TextFormulaOCR(object):
    ...
    def recognize(
        ...
        # Mask out the formula part and OCR the other parts
        for mf_box_info in analyzer_outs:
        ...
        for line_box_info in text_box_infos['detected_texts']:
            # crop_img_info['box'] may be an angled rectangle that needs to be converted to a horizontal rectangle
        ...
        for idx, line_boxes in enumerate(outs):
            if (
                ...
                if line_boxes[-1]['text'].lower() == 'o':
                    line_boxes[-1]['text'] = '。'
            if len(line_boxes) > 1:
                # Remove extra punctuation on borders
                ...
                # Merge the formula tag into the formula
                for _idx2, box in enumerate(line_boxes[1:]):
                    if (
                        box['type'] == 'text'
    ...
    def _split_line_image(cls, line_box, embed_mfs):
        # Use the position of embedding formula to cut a single line of text and image into multiple small segments, and then these small segments will be thrown into OCR for text recognition
        line_box = line_box[0]
        if not embed_mfs:
