    ...
    def _cal_scores(self, outs):
        if isinstance(outs, GenerateBeamEncoderDecoderOutput):
            ...
            for idx, example in enumerate(scores):
                ...
                assert cur_length > 1
                # Get the geometric mean. Note: The first element in example corresponds to the second element in the sequence
                mean_probs.append(
...
def remove_empty_text(latex_expression):
    ...
    patterns = [
        ...
        r'_\s*{\s*}',  # Match _{}
        r'\\text\s*{\s*}',  # match \text{}
        r'\\tilde\s*{\s*}',  # matching \tilde{}
        r'\\bar\s*{\s*}',  # match \bar{}
        r'\\vec\s*{\s*}',  # match \vec{}
        r'\\acute\s*{\s*}',  # match \acute{}
        r'\\grave\s*{\s*}',  # matching \grave{}
        r'\\breve\s*{\s*}',  # matching \breve{}
        r'\\overline\s*{\s*}',  # matching \overline{}
        r'\\dot\s*{\s*}',  # match \dot{}
        r'\\ddot\s*{\s*}',  # Match \ddot{}
        r'\\widehat\s*{\s*}',  # matching \widehat{}
        r'\\widetilde\s*{\s*}',  # matching \widetilde{}
    ]

    # Use the sub function to replace
...
latex_whitespace_symbols = [
    r'\\ +',  # Spaces
    r'\\quad\s*',  # 1em width of blank
    r'\\qquad\s*',  # 2em width of the blank
    r'\\,\s*',  # Narrow spaces
    r'\\:\s*',  # Medium space
    r'\\;\s*',  # Large spaces
    r'\\enspace\s*',  # 1em blank
    r'\\thinspace\s*',  # 1/6em blank
    r'\\!\s*',  # Exclamation mark
]


def remove_trailing_whitespace(latex_str):
    # Defines a regular expression pattern that matches trailing whitespace characters
...
def find_all_left_or_right(latex, left_or_right='left'):
    ...
    # Match all '\left' followed by the first non-whitespace character in latex and locate where they are
    for m in re.finditer(rf'\\{left_or_right}\s*\S', latex):
        start, end = m.span()
        if latex[end - 1] in string.ascii_letters:
            continue
        # If the last character is "\", the match continues until a non-alphabetic character is matched
        # e.g. "\left \big("
        ...
        # FIXME: There may be multiple '\left' in the ori_str, which need to be separated
...
def match_left_right(left_str, right_str):
    """Match the left and right parentheses, such as matching `\left(` and `\right)`。"""
    left_str = left_str.strip().replace(' ', '')[len('left') + 1 :]
    right_str = right_str.strip().replace(' ', '')[len('right') + 1 :]
    # Remove the same part at the beginning
...
def fix_latex(latex: str) -> str:
    """The identification results are further processed and corrected."""
    # # Replace all Chinese parentheses in latex with English parentheses
    # latex = latex.replace('（', '(').replace('）', ')')
    # # Replace all Chinese commas in latex with English commas
    # latex = latex.replace('，', ',')
    ...
    # left and right Find the matching, left finds the right matching that is ahead of it and closest to it
    ...
    for left_bracket_info in left_bracket_infos:
        # Replace unmatched '\left' with spaces of equal length
        left_len = len('left') + 1
    ...
    for right_bracket_info in right_bracket_infos:
        # Replace the unmatched '\right' with a space of equal length
    ...
    # Replace consecutive spaces in latex with a single space
