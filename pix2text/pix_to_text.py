...
class Pix2Text(object):
    ...
    def _parse_remaining(self, img0, layout_out, column_meta, debug_dir, **kwargs):
        ...
        # Mask out the parts that have been parsed by the layout parser, and then OCR the other parts
...
def _separate_blocks(col_outs, spellchecker, **kwargs):
    ...
    for _box_info in col_outs[1:]:
        ...
        if (
            ...
            # If the distance between the current row and the next row is less than the height of the row, they are considered to be in the same block
